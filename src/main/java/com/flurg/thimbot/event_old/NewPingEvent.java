 ...
 */
public final class NewPingEvent extends NewProtocolCommandEvent {
    public NewPingEvent(final String source, final String rawMessage, final List<String> targets, final boolean outbound) {
        super(source, rawMessage, targets, outbound);
    }

    public boolean isAcknowledgedBy(final NewEvent inboundEvent, final String myNick) {
        return inboundEvent instanceof NewPongEvent && getRawMessage().equals(inboundEvent.getRawMessage());
    }
